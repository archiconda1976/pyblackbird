...
def _format_set_zone_source(
    profile: BlackbirdProfile, zone: int, source: int
) -> bytes:
    if profile.legacy_protocol:
...
def get_blackbird(
    ...
    :param port: TCP port when ``use_serial`` is false. Defaults to the profile port.
    ...
    class BlackbirdSync(Blackbird):
        ...
        def zone_status(self, zone: int):
            ...
            return ZoneStatus.from_string(
                ...
                self._process_request(
                    _format_zone_status_request(self.profile, zone), skip=20
                ),
            )
        ...
        def set_zone_source(self, zone: int, source: int):
            ...
            self._process_request(_format_set_zone_source(self.profile, zone, source))
...
async def get_async_blackbird(
    ...
    class BlackbirdAsync(Blackbird):
        ...
        async def zone_status(self, zone: int):
            ...
            string = await self._protocol.send(
                _format_zone_status_request(self.profile, zone), skip=15
            )
        ...
        async def set_zone_source(self, zone: int, source: int):
            self.profile.validate_zone(zone)
            self.profile.validate_source(source)
            await self._protocol.send(_format_set_zone_source(self.profile, zone, source))
